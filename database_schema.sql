...
CREATE TABLE asset_formats (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(255) NOT NULL,
    type format_type NOT NULL, -- 'resizing' or 'repurposing'
    platform_id UUID REFERENCES repurposing_platforms(id) ON DELETE CASCADE, -- Nullable, only for 'repurposing' type
    category VARCHAR(50), -- 'Mobile', 'Web', etc. Only for 'resizing' type
    width INT NOT NULL,
    height INT NOT NULL,
    is_active BOOLEAN DEFAULT TRUE,
    created_by_admin_id UUID REFERENCES users(id) ON DELETE SET NULL,
